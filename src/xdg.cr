 module XDG
   ...
   MACOS_APP_SUPPORT = File.join(Dir.home, "Library", "Application Support")
   MACOS_PREFERENCES = File.join(Dir.home, "Library", "Preferences")
   ...
   def self.valid_runtime_dir?(path : String) : Bool
     ...
     info = File.info(path)
     info.directory? && info.owner == Process.uid && info.permissions.owner_write?
   end
   ...
   private def self.default_config_home : String
     ...
     {% elsif flag?(:darwin) %}
       macos_app? ? MACOS_PREFERENCES : File.join(Dir.home, ".config")
     {% else %}
   ...
   private def self.default_data_home : String
     ...
     {% elsif flag?(:darwin) %}
       macos_app? ? MACOS_APP_SUPPORT : File.join(Dir.home, ".local", "share")
     {% else %}
   ...
   private def self.default_cache_home : String
     {% if flag?(:win32) %}
       File.join(ENV["LOCALAPPDATA"]? || File.join(Dir.home, "AppData", "Local"), "cache")
     {% elsif flag?(:darwin) %}
